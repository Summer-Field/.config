...
function definingReferentialAction(wordsBeforePosition) {
    const lastWord = wordsBeforePosition[wordsBeforePosition.length - 2];
    return (lastWord != undefined &&
...
function getSuggestionsForRelationDirective(wordsBeforePosition, currentLineUntrimmed, lines, document, block, position, binPath) {
    // create deep copy
    ...
    if (definingReferentialAction(wordsBeforePosition)) {
        const suggestions = referentialActions_1.default(binPath, document.getText()).map((action) => vscode_languageserver_1.CompletionItem.create(action));
        return {
            items: suggestions,
            isIncomplete: false,
        };
...
function getSuggestionsForInsideAttributes(untrimmedCurrentLine, lines, document, position, block, binPath) {
    let suggestions = [];
    ...
    if (wordBeforePosition.includes('@default')) {
        ...
        return getSuggestionsForRelationDirective(wordsBeforePosition, untrimmedCurrentLine, lines, document, block, position, binPath);
    }
    return {
